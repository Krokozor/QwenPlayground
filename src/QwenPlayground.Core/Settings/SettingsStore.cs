...
public sealed class AppSettings
{
    ...
    /// <summary>
    /// KV-якорь при спавне субагентов: save/restore KV-кеша main-агента в
    /// sessions/&lt;id&gt;-kv.bin вокруг синхронного хода субагента (см. SubagentSpawner).
    /// Выкл — субагент работает без якоря (возврат main'а может стоить пере-евалюации
    /// контекста). Выключать при нехватке места на диске (снапшот ~320 КБ/токен) или
    /// при копировании папки проекта (якорь — гигабайты рядом с сессиями). Дефолт true.
    /// </summary>
    public bool KvCacheEnabled { get; set; } = true;

    /// <summary>
    /// Хранить ли полный рендеренный промпт каждого хода в истории (Generation.Prompt в chat.json).
    /// По умолчанию ВЫКЛ: промпт — копия растущего чата на каждое сообщение, раздувает файл
    /// до десятков МБ. Вкл — кнопка «промпт» у assistant-сообщений показывает точный промпт +
    /// сырой вывод (диагностика форматирования/поведения). Старые сессии с сохранённым промптом
    /// работают без изменений (обратная совместимость): кнопка появляется, где промпт есть.
    /// </summary>
